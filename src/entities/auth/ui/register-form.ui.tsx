...
import LoginBg from "../../../../public/statics/login-bg.jpg";
    ...
    return (
        <div className={`${className} flex items-stretch flex-col sm:flex-row overflow-hidden bg-zinc-800 rounded-lg`}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 flex-1 grid gap-y-4">
                <div className="text-center text-xl sm:text-3xl text-white font-medium" id="form-heading">{label}</div>
                <div className="w-full grid gap-y-2" id="form-group-1">
                    <Label htmlFor="name">Имя</Label>
                    <Input {...register('name')} className={errors.name ? 'border border-red-500' : ''} type="text" placeholder="Иван" id="name" />
                    {errors.name && (
                ...
                <div className="w-full grid gap-y-2" id="form-group-2">
                    ...
                    <Input {...register('email')} type="email" placeholder="user@example.ru" id="email" />
                    {errors.email && (
                ...
                <div className="pt-5 w-full grid gap-y-4" id="form-group-3">
                    ...
                    <Link href={String(loginUrl)} className="text-gray-400 transition-colors hover:text-white">Войти в личный кабинет</Link>
                </div>
            </form>
        </div>
    )
